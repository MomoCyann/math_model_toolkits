...
class NewStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        # 理一下逻辑，两个ranks记录了应该买入，应该卖出的股票，没有信号（0）已经被剔除
        # ranks每次与rtop(前一天持有的)做并集作为新的rtop，表示当天应该持有的所有股票
        # 再检查rtop里面有没有在ranks_sold的股票，应该卖出，有的话从rtop里删掉。
        # 这样只根据rtop来平分仓位。
        ...
        # 完成合并操作，合并后是集合，储存data，地址应该是一样的，可以去并集。
        ...
        # 从rtop(持有)里剔除rbot(卖出信号)的股票
        for d in self.rtop:
            if d in self.rbot:
                self.rtop.remove(d)
        # 记录最大持有股票数
        print('今天是：', self.datetime.date(0))
        print('今天购买股票数目为：', len(self.rtop))
